...
async def setup_and_run_simulation(
    state: MDCrowState,
    pdb_id: str,
    ...
    Args:
        pdb_id (str): ID of the PDB/CIF file to be used in the simulation.
        forcefield_files (List[str]): List of forcefield files to be applied.
        save (bool, optional): Whether to save log files and trajectories. Defaults to True.
            - If True, the simulation results will be stored.
            - If False, the simulation is considered as a test run or script development phase.
        system_params (Dict[str, Any], optional): Parameters for configuring the OpenMM system. Defaults to:
            - "nonbondedMethod": "NoCutoff" (Other options: "CutoffNonPeriodic", "CutoffPeriodic", "Ewald", "PME")
            - "nonbondedCutoff": "1 * nanometers" (Required for methods other than "NoCutoff")
            - "ewaldErrorTolerance": None (Required if "PME" is chosen)
            - "constraints": "None" (Other options: "HBonds", "AllBonds", "OnlyWater")
            - "rigidWater": False
            - "constraintTolerance": None (Required if using constraints)
            - "solvate": False (Set to True if solvation is needed)
        integrator_params (Dict[str, Any], optional): Parameters for the OpenMM integrator. Defaults to:
            - "integrator_type": "LangevinMiddle"
            - "Temperature": "300 * kelvin"
            - "Friction": "1.0 / picoseconds"
            - "Timestep": "0.002 * picoseconds"
            - "Pressure": "1.0 * bar"
        simulation_params (Dict[str, Any], optional): Parameters for the OpenMM simulation. Defaults to:
            - "Ensemble": "NVT" (Other options: "NPT", "NVE")
            - "Number of Steps": 5000 (Total simulation steps)
            - "record_interval_steps": 100 (Interval between records, recommended: steps/100)
            - "record_params": ["step", "potentialEnergy", "temperature"]
                (Other available parameters: "Time", "Speed", "Progress", "RemainingTime",
                "ElapsedTime", "PotentialEnergy", "KineticEnergy", "TotalEnergy",
    ...
    print(input_args)
    input = check_system_params(input_args)
    print(input)
    error = input.get("error", None)
    if error:
        print(f"error found: {error}")
        return "Failed. " + error, 0, False

    try:
        pdb_id = input["pdb_id"]
        # check if pdb_id is in the registry or as 1XYZ_112233 format
        if pdb_id not in state.path_registry.list_path_names():
            return (
                (
    ...
    try:
        ...
        sim_id = state.path_registry.get_fileid(file_name, FileType.SIMULATION)
    except Exception as e:
    ...
    try:
        openmmsim = OpenMMSimulation(input, state.path_registry, save, sim_id, pdb_id)
    ...
    try:
        ...
        state.path_registry.map_path(
            sim_id,
            f"{state.path_registry.ckpt_simulations}/{file_name}",
        ...
        if save:
            ...
            for record in records:
                record[0] = state.path_registry.get_fileid(  # Step necessary here to
                    record[1].split("/")[-1],  # avoid id being repeated
                    FileType.RECORD,
                )
                state.path_registry.map_path(*record)
        return (
            (
...
def _parse_parameter(parameter, default_unit, possible_units):
    """
...
def parse_temperature(temperature):
    ...
    return _parse_parameter(temperature, "k", possible_units)


def parse_friction(friction):
    possible_units = {
    ...
    return _parse_parameter(friction, "1/ps", possible_units)


def parse_timestep(timestep):
    possible_units = {
        "ps": unit.picoseconds,
    ...
    return _parse_parameter(timestep, "ps", possible_units)
...
def parse_pressure(pressure):
    ...
    return _parse_parameter(pressure, "bar", possible_units)


def _process_parameters(user_params, param_type="system_params"):
    """
    ...
    if param_type == "system_params":
        for key, value in user_params.items():
            ...
            if key == "nonbondedCutoff" or key == "nonbondedcutoff":
                try:
                    processed_params[key] = _parse_cutoff(value)
                except ValueError as e:
...
def check_system_params(values):
    ...
    if system_params:
        system_params, msg = _process_parameters(
    ...
    if integrator_params:
        integrator_params, msg = _process_parameters(
