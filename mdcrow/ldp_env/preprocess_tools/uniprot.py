...
def MapProteinRepresentation(
    state: MDCrowState, query: str, src_db: str = "UniProtKB_AC-ID", dst_db: str = "PDB"
):
    """
    ...
    Args:
        state: The state of the MDCrow environment
        query: The protein identifier to map
        src_db: The source database to map from (default: 'UniProtKB_AC-ID')
        dst_db: The target database to map to (default: 'PDB')
    """
    uniprot = QueryUniprot()
    try:
        mapped_ids = uniprot.get_sequence_mapping(query, from_db=src_db, to_db=dst_db)
        return str(mapped_ids), 0, False
...
def UniprotID2Name(
    state: MDCrowState, query: str, primary_accession: str = "", all_names: bool = True
):
    """
    Get the protein name for a specific protein, with the option to filter by
    primary accession number. If you have the primary accession number, you can
    use it to filter the results. Otherwise, all names associated with the
    protein will be returned. Input the uniprot ID of the protein.
    ...
    Args:
        ...
        primary_accession: The primary accession number of the protein.
            Defaults to None.
        all_names: Whether to include short names in the results. Defaults to True.
    """
    uniprot = QueryUniprot()
    try:
        names = uniprot.get_protein_name(
            query,
            primary_accession=primary_accession,
            short_names=all_names,
            alternative_names=all_names,
        )
        return ", ".join(names), 0, False
    except Exception as e:
        return str(e), 0, False
...
def GetBindingSites(state: MDCrowState, query: str, primary_accession: str = ""):
    """
    Get the binding sites known for a specific protein, given the primary
    accession number. Both the query string and primary accession number are
    required.
    ...
    uniprot = QueryUniprot()
    try:
        sites = uniprot.get_relevant_sites(query, primary_accession, "binding")
        return str(sites), 0, False
    except Exception as e:
        return str(e), 0, False
...
def GetActiveSites(state: MDCrowState, query: str, primary_accession: str = ""):
    """
    Get the active sites known for a specific protein, given the primary
    accession number. Both the query string and primary accession number are
    required.
    ...
    uniprot = QueryUniprot()
    try:
        sites = uniprot.get_relevant_sites(query, primary_accession, "active")
        return str(sites), 0, False
    except Exception as e:
        return str(e), 0, False
...
def GetRelevantSites(state: MDCrowState, query: str, primary_accession: str = ""):
    """
    Get the relevant sites for a specific protein, given the primary
    accession number. You must provide the query string and primary
    accession number. The relevant sites are sites that are known to be
    important for the protein's function, but are not necessarily active
    or binding sites.
    ...
    uniprot = QueryUniprot()
    try:
        sites = uniprot.get_relevant_sites(query, primary_accession, "sites")
        return str(sites), 0, False
    except Exception as e:
        return str(e), 0, False


def GetAllKnownSites(state: MDCrowState, query: str, primary_accession: str = ""):
    """
    Get all known sites for a specific protein, given the primary
    accession number. You must provide the query string and primary
    accession number. This tool is a one-stop shop to get all known sites
    for the protein, including active sites, binding sites, and other
    relevant sites.
    ...
    uniprot = QueryUniprot()
    try:
        active_sites = uniprot.get_relevant_sites(query, primary_accession, "active")
        active_sites_msg = (
        ...
        binding_sites = uniprot.get_relevant_sites(query, primary_accession, "binding")
        binding_sites_msg = (
        ...
        sites = uniprot.get_relevant_sites(query, primary_accession, "sites")
        sites_msg = (
            f"Other Relevant Sites: {sites}" if sites else "No other relevant sites."
        )
...
def GetProteinFunction(state: MDCrowState, query: str, primary_accession: str = ""):
    """
    Get the protein function for a specific protein, with the option to
    filter by primary accession number. If you have the primary accession
    number, you can use it to filter the results. Otherwise, all functions
    associated with the protein will be returned. Input the uniprot ID of
    the protein.
    ...
    uniprot = QueryUniprot()
    try:
        functions = uniprot.get_protein_function(
            query, primary_accession=primary_accession
        )
        return ", ".join(functions), 0, False
    except Exception as e:
        return str(e), 0, False
...
def GetProteinAssociatedKeywords(
    state: MDCrowState, query: str, primary_accession: str = ""
):
    """
    Get the keywords associated with a specific protein, with the option
    to filter by primary accession number. If you have the primary
    accession number, you can use it to filter the results. Otherwise,
    all keywords associated with the protein will be returned. Input the
    ...
    uniprot = QueryUniprot()
    try:
        keywords = uniprot.get_keywords(query, primary_accession=primary_accession)
        return ", ".join(keywords), 0, False
    except Exception as e:
        return str(e), 0, False
...
def GetAllSequences(state: MDCrowState, query: str):
    """
    Get all the sequences for a specific protein. Input the uniprot ID of
    the protein. This tool will return all sequences associated with the
    protein.
    ...
    uniprot = QueryUniprot()
    try:
        sequences = uniprot.get_all_sequences(query)
        return ", ".join(sequences), 0, False
    except Exception as e:
        return str(e), 0, False
...
def GetInteractions(state: MDCrowState, query: str, primary_accession: str = ""):
    """
    Get the interactions for a specific protein, given the primary
    accession number. Both the query string and primary accession number
    are required. This tool will return the interactions for the protein.
    ...
    uniprot = QueryUniprot()
    try:
        interactions = uniprot.get_interactions(query, primary_accession)
        return str(interactions), 0, False
    except Exception as e:
        return str(e), 0, False
...
def GetSubunitStructure(state: MDCrowState, query: str, primary_accession: str = ""):
    """
    Get the subunit structure information for a specific protein, given the
    primary accession number. Both the query string and primary accession
    number are required. This tool will return the subunit structure
    ...
    uniprot = QueryUniprot()
    try:
        structure_info = uniprot.get_subunit_structure(query, primary_accession)
        return str(structure_info), 0, False
    except Exception as e:
        return str(e), 0, False
...
def GetSequenceInfo(state: MDCrowState, query: str, primary_accession: str = ""):
    """
    Get the sequence information for a specific protein, given the primary
    accession number. Both the query string and primary accession number are
    required. This tool will return the sequence, length, and molecular
    weight.
    ...
    uniprot = QueryUniprot()
    try:
        sequence_info = uniprot.get_sequence_info(query, primary_accession)
        # remove crc64 and md5 keys, as they are not useful to the agent
        sequence_info.pop("crc64", None)
        sequence_info.pop("md5", None)
        return str(sequence_info), 0, False
    except Exception as e:
        return str(e), 0, False
...
def GetPDBProcessingInfo(
    state: MDCrowState, query: str, processing_type: str, primary_accession: str = ""
):
    """
    Get the processing information for a specific protein, given the primary
    accession number. Both the query string and primary accession number are
    required. Input the query, accession number, and the type of processing
    information to retrieve (e.g., chain, crosslink, disulfide-bond, etc.).
    Here is a list of the processing types you can retrieve: chain,
    crosslink, disulfide-bond, glycosylation, initiator-methionine,
    lipidation, modified-residue, peptide, propeptide, signal-peptide,
    transit-peptide

    Args:
        state: The state of the MDCrow environment
        query: The protein identifier to map
        processing_type: The type of processing information to retrieve
        primary_accession: The primary accession number of the protein.
            Defaults to None.
    """
    uniprot = QueryUniprot()
    try:
        processing_info = uniprot.get_ptm_processing_info(
            query, primary_accession, processing_type
        )
        return str(processing_info), 0, False
    except Exception as e:
        return str(e), 0, False
...
def GetPDB3DInfo(state: MDCrowState, query: str, primary_accession: str = ""):
    """
    Get the 3D structure information for a specific protein, given the
    primary accession number. Both the query string and primary accession
    number are required. This tool will return information from the PDB
    database for the protein, including the PDB ID, chain, and resolution.
    ...
    uniprot = QueryUniprot()
    try:
        structure_info = uniprot.get_3d_info(query, primary_accession)
        return str(structure_info), 0, False
    except Exception as e:
        return str(e), 0, False
...
def GetTurnsBetaSheetsHelices(
    state: MDCrowState, query: str, primary_accession: str = ""
):
    """
    Get the number and location of turns, beta sheets, and helices for a
    specific protein, given the primary accession number. Both the query
    string and primary accession number are required. This tool will
    return the number and location of turns, beta sheets, and helices for
    the protein.
    ...
    uniprot = QueryUniprot()
    try:
        turns = uniprot.get_structure_info(query, primary_accession, "turn")
        beta_sheets = uniprot.get_structure_info(query, primary_accession, "beta")
        helices = uniprot.get_structure_info(query, primary_accession, "helix")
        return (
            f"Turns: {turns}\nBeta sheets: {beta_sheets}\nHelices: {helices}",
            0,
            False,
...
def GetUniprotID(
    ...
    """
    Get the UniProt ID for a specific protein. Input the query string of
    the protein. This tool will return the UniProt ID of the protein.
    You can optionally specify whether you want to return all IDs or just
    one ID. By default, a single ID will be returned.

    Args:
        state: The state of the MDCrow environment
        query: The protein identifier to map
        all_ids: Whether to return all IDs or just one ID. Defaults to False.
        include_uniprotkbids: Whether to include UniProtKB IDs in the results.
            Defaults to False.
    """
    uniprot = QueryUniprot()
    try:
        ids = uniprot.get_ids(
            query,
            single_id=not all_ids,
            include_uniprotkbids=include_uniprotkbids,
        )
        return ", ".join(ids), 0, False
    except Exception as e:
        return str(e), 0, False
...
def GetGeneNames(state: MDCrowState, query: str, primary_accession: str = ""):
    """
    Get the gene names associated with a specific protein, with the
    option to filter by primary accession number. If you have the
    primary accession number, you can use it to filter the results.
    Otherwise, all gene names associated with the protein will be
    returned. Input the uniprot ID of the protein.
    ...
    uniprot = QueryUniprot()
    try:
        gene_names = uniprot.get_gene_names(query, primary_accession=primary_accession)
        return ", ".join(gene_names), 0, False
    except Exception as e:
        return str(e), 0, False
...
def GetKineticProperties(state: MDCrowState, query: str, primary_accession: str = ""):
    """
    Get the kinetics information for a specific protein, given the primary
    accession number. Both the query string and primary accession number
    are required.
    ...
    uniprot = QueryUniprot()
    try:
        kinetics = uniprot.get_kinetics(query, primary_accession)
        return str(kinetics), 0, False
    except Exception as e:
        return str(e), 0, False
