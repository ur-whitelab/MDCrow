...
def compute_solvent_accessible_surface_area(
    state: MDCrowState,
    top_fileid: Optional[float] = None,
    traj_fileid: Optional[float] = None,
    ...
    try:
        sasa_analysis = SASAFunctions(
            state.path_registry, top_fileid, traj_fileid, molecule_name
        )
        return f"Succeeded. {sasa_analysis.plot_sasa()}", 0, False
