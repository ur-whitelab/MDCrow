...
def compute_salt_bridges(
    ...
    threshold_distance: float = 0.4,
    pH_dependence: bool = False,
    target_pH: float = 7.0,
    specific_frame: Optional[int] = None,
    acidic_residues: Tuple[str, ...] = ("ASP", "GLU"),
    basic_residues: Tuple[str, ...] = ("ARG", "LYS", "HIS"),
    ...
    Args:
        traj_file (str): File ID of the trajectory file to be analyzed.
        top_file (Optional[str], optional): File ID of the topology file associated with the trajectory. Defaults to None.
        threshold_distance (float, optional): Maximum distance (in nanometers) between
            charged groups to be considered a salt bridge. Defaults to 0.4 nm.
        pH_dependence (bool, optional): Whether to consider pH-dependent protonation states
            for histidine residues. Defaults to False.
        target_pH (float, optional): The target pH level to determine histidine protonation.
            Used only if `pH_dependence` is True. Defaults to 7.0.
        specific_frame (Optional[int], optional): If provided, the analysis will be performed
            on a specific frame from the trajectory instead of the entire simulation.
        acidic_residues (Tuple[str, ...], optional): List of acidic residues to be considered
            in the salt bridge analysis. Defaults to ("ASP", "GLU").
        basic_residues (Tuple[str, ...], optional): List of basic residues to be considered
            in the salt bridge analysis. Defaults to ("ARG", "LYS", "HIS").
        include_terminals (bool, optional): Whether to include N-terminal and C-terminal
            charged groups in the salt bridge analysis. Defaults to True."
            """

    try:
        if state.path_registry is None:
            return "Path registry is not set"

        salt_bridge_function = SaltBridgeFunction(state.path_registry)
        ...
        return "Succeeded. " + message, 0, False
