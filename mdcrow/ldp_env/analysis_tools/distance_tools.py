...
def compute_distance(
    ...
    topology_fileid: str,
    analysis: Literal["all", "not all"] = "all",
    mode: Literal["CA", "COM"] = "CA",
    selection1: Optional[str] = None,
    ...
    try:
        input = validate_distance_inputs(state.path_registry, **input)
    except ValueError as e:
        return f"Error using the PCA Tool: {str(e)}", 0, False
    ...
    utils = DistanceToolsUtils(path_registry=state.path_registry)
    if mode == "CA":
    ...
    path = f"{state.path_registry.ckpt_figures}/dist_{trajectory_id}/\
        dist_matrix_{trajectory_id}.gif"
    if not os.path.exists(f"{state.path_registry.ckpt_figures}/dist_{trajectory_id}"):
        os.makedirs(f"{state.path_registry.ckpt_figures}/dist_{trajectory_id}")
    fig_id = state.path_registry.get_fileid(file_name=path, type=FileType.FIGURE)
    movie_desc = utils.make_movie(dist_matrix, path=path, source_id=trajectory_id)
    state.path_registry.map_path(fig_id, path, movie_desc)
    # save some of the frames as images
    utils.save_matrix_frame(
        dist_matrix, f"{state.path_registry.ckpt_figures}/dist_{trajectory_id}"
    )

    return (
        "Succeeded. Distance Matrix created with ID: " + fig_id + system_message,
        0,
        False,
    )
...
def compute_contacts(
    ...
    Args:
        trajectory_fileid (str): File ID of the trajectory file to be analyzed.
        topology_fileid (str): File ID of the topology file associated with the trajectory.
        selection (str): Specifies which residues to include in the contact analysis.
            - "all": Calculates distances between all residue pairs.
            - Example selections:
                - "resid 0 to 10" (selects residues from 0 to 10)
...
"""
    ...
    path = f"{state.path_registry.ckpt_figures}/contact_{trajectory_id}/\
        contact_matrix_{trajectory_id}.gif"
    if not os.path.exists(
        f"{state.path_registry.ckpt_figures}/contact_{trajectory_id}"
    ):
        os.makedirs(f"{state.path_registry.ckpt_figures}/contact_{trajectory_id}")

    fig_id = state.path_registry.get_fileid(file_name=path, type=FileType.FIGURE)
    movie_desc = utils.make_movie(matrix, path=path, source_id=trajectory_id)
    state.path_registry.map_path(fig_id, path, movie_desc)
    utils.save_matrix_frame(
        matrix, f"{state.path_registry.ckpt_figures}/contact_{trajectory_id}"
    )
    return "Contact Matrix Figure created with ID: " + fig_id, 0, False
