...
def compute_radius_of_gyration(
    ...
    trajectory and returns the average. Give this tool \
    BOTH the trajectory file ID and the topology file ID.
    Args:
        traj_file (str): File of the trajectory file to be analyzed.
        top_file (str): File of the topology file associated with the trajectory.
    """

    rgy = RadiusofGyration(state.path_registry)
    try:
        rgy._load_traj(top_file=top_file, traj_file=traj_file)
    except Exception as e:
        return f"Error loading traj: {e}", 0, False
    try:
        return "Succeeded. " + rgy.compute_plot_return_avg(), 0, False
    except Exception as e:
        return f"Failed Computing RGY: {e}", 0, False
