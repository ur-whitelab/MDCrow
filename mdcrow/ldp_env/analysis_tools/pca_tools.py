...
def run_pca_analysis(
    ...
    try:
        ...
        if analysis.lower() == "cosine_convergence":
            ...
            return f"Succeeded. Analyses done: {result}. \n\n {system_input_message}"
...
def perform_pca_analysis(
    ...
    topology_fileid: str,
    pc_percentage: Optional[float] = 95.0,
    analysis: str = "all",
    selection: Optional[str] = "backbone and name CA",
):
    """
    Performs Principal Component Analysis (PCA) on a molecular simulation trajectory.
    ...
    Args:
        ...
        pc_percentage (Optional[float], optional): Maximum cumulative percentage of
            principal components to include in the analysis. Defaults to 95.0.
        analysis (str, optional): Type of PCA analysis to perform. Available options:
            - "scree_plot": Saves a scree plot of the eigenvalues.
            - "pca_analysis": Computes principal components and saves a grid plot for visualization.
            - "Cosine": Measures the cosine convergence of the top 3 principal components.
            - "all": Performs all of the above analyses.
            Defaults to "all".
        selection (Optional[str], optional): Specifies which atoms to include in the PCA analysis.
            Defaults to "backbone and name CA".
    ...
    try:
        input = validate_input(path_registry, **input)

    except ValueError as e:
        return f"Failed. Error using the PCA Tool: {str(e)}", 0, False
